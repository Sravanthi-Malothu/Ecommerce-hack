...
const COMPLEMENTARY_MAPPING = {
  ...
  prod_casual_sneakers: [
    { name: 'No-Show Organic Cotton Socks', category: 'Footwear', price: 450, affinityScore: 0.86 },
    { name: 'Sneaker Cleaner Kit', category: 'Footwear', price: 750, affinityScore: 0.80 }
  ],
  prod_winter_parka: [
  ...
  prod_leather_jacket: [
    { name: 'Napa Leather Conditioner Spray', category: 'Apparel', price: 950, affinityScore: 0.89 },
    { name: 'Silk Neck Scarf', category: 'Apparel', price: 1500, affinityScore: 0.79 }
  ],
  ...
  prod_office_chair: [
    { name: 'Memory Foam Lumbar Support Pillow', category: 'Home Goods', price: 1600, affinityScore: 0.90 },
    { name: 'Ergonomic Footrest Stool', category: 'Home Goods', price: 1900, affinityScore: 0.81 }
  ],
  prod_espresso_maker: [
    { name: 'Stainless Steel Milk Pitcher', category: 'Home Goods', price: 1100, affinityScore: 0.93 },
    { name: 'Precision Espresso Tamper', category: 'Home Goods', price: 1400, affinityScore: 0.87 }
  ],
  prod_dutch_oven: [
  ...
  prod_robot_vacuum: [
    { name: 'Replacement HEPA Filter 4-Pack', category: 'Home Goods', price: 1200, affinityScore: 0.94 },
    { name: 'Dual Edge Side Brushes', category: 'Home Goods', price: 850, affinityScore: 0.89 }
  ],
  ...
  prod_hair_dryer_styler: [
    { name: 'Heat Protectant Styling Spray', category: 'Beauty & Care', price: 950, affinityScore: 0.93 },
    { name: 'Ceramic Barrel Round Brush', category: 'Beauty & Care', price: 1200, affinityScore: 0.87 }
  ],
  prod_camping_tent: [
  ...
  prod_earbuds_wireless: [
    { name: 'Silicone Ear-Tip Replacement Set', category: 'Electronics', price: 550, affinityScore: 0.91 },
    { name: 'Carabiner Protective Charging Case', category: 'Electronics', price: 850, affinityScore: 0.88 }
  ]
};

/**
 * Cross-Product Promotion & Uplift Learning Engine
 * Generates co-purchase bundle recommendations, tracks attachment rates, calculates incremental profit/revenue,
...
export function generateCrossProductBundlesData() {
  ...
  PRODUCTS.forEach((product, idx) => {
    const comps = COMPLEMENTARY_MAPPING[product.product_id] || [
      { name: 'Universal Care Kit', category: product.category, price: 800, affinityScore: 0.80 }
    ];

    const bundledItemNames = comps.map(c => c.name);
    const bundleDiscountPct = idx % 4 === 0 ? 20 : idx % 3 === 0 ? 15 : 10;
    
    // Performance metrics
    const baseUnits = 320 + (idx * 20);
    const attachmentRatePct = +(45 + (comps[0].affinityScore * 35) - (idx % 3) * 8).toFixed(1);
    
    const isHighAttachment = attachmentRatePct > 55;
    const isModerateAttachment = attachmentRatePct >= 40 && attachmentRatePct <= 55;

    const promoUnits = Math.round(baseUnits * (1 + (attachmentRatePct * 0.012)));
    ...
    const unitCost = Math.round(totalBundleListPrice * 0.50);
    ...
    const promoCostTotal = Math.round(promotedRevenue * 0.08) + 1500;
    const promotionRoiPct = promoCostTotal > 0 ? +((incrementalProfit / promoCostTotal) * 100).toFixed(1) : 0;

    // Meaningful Uplift Criteria (+20% profit lift & > 45% attachment rate)
    const meaningfulUpliftFlag = incrementalProfit > 15000 && attachmentRatePct > 45;

    // AI Future Bundle Verdict
    let repeatRecommendation = 'RECOMMENDED_FOR_FUTURE';
    let verdictTitle = '🟢 Recommended for Future Bundling';
    let verdictExplanation = '';

    if (meaningfulUpliftFlag && isHighAttachment) {
      repeatRecommendation = 'RECOMMENDED_FOR_FUTURE';
      verdictTitle = '🟢 High-Performance Co-Promote Bundle';
      verdictExplanation = `Exceptional cross-product uplift! High ${attachmentRatePct}% attachment rate generated +₹${incrementalProfit.toLocaleString('en-IN')} in incremental profit (+${salesUpliftPct}% sales lift). Highly recommended for future promotional campaigns.`;
    } else if (isModerateAttachment || incrementalProfit > 0) {
      repeatRecommendation = 'MODIFY_DISCOUNT';
      verdictTitle = '🟡 Modify Discount Depth Before Repeating';
      verdictExplanation = `Moderate attachment rate (${attachmentRatePct}%). While sales volume grew by +${salesUpliftPct}%, discount depth of ${bundleDiscountPct}% reduced margin dollar retention. Lower bundle discount to 10% to protect category profit.`;
    } else {
