    ...
    <div className="space-y-6">
      ...
      <div className="saas-card p-6 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
              Co-Purchase Affinity &amp; Attachment Rate AI
            </span>
          </div>
          <p className="text-xs text-slate-300">
            Identify complementary co-purchased accessories (e.g. *Smartwatch + Charger + Case*), calculate attachment rates, measure incremental profit lift, and learn which bundles to recommend or avoid in future promotions.
          </p>
        </div>
        ...
        <div className="bg-indigo-500/10 border border-indigo-500/30 p-3 rounded-2xl flex items-center gap-3 shrink-0">
          ...
          <div>
            <div className="text-xs font-bold text-indigo-200">Incremental Profit Optimizer</div>
            <div className="text-[10px] text-slate-300">Tests for meaningful sales &amp; margin uplift</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="saas-card p-5 border-indigo-100 bg-gradient-to-br from-indigo-50/50 to-white">
          ...
          <p className="text-[11px] text-slate-500 mt-1">
            Net profit generated over baseline single sales
          </p>
        </div>
        ...
        <div className="saas-card p-5 border-purple-100 bg-gradient-to-br from-purple-50/50 to-white">
          ...
          <p className="text-[11px] text-slate-500 mt-1">
            Bundles meeting +20% profit &amp; &gt;45% attachment criteria
          </p>
        </div>
        ...
        {activeBundle && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            ...
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80 flex flex-col justify-between space-y-2">
              ...
              <div>
                <span className="text-[11px] font-semibold text-slate-400">Incremental Profit Lift</span>
                <div className="text-xl font-black text-emerald-400">+₹{activeBundle.incrementalProfit.toLocaleString('en-IN')}</div>
              </div>

              <div>
                <span className="text-[11px] font-semibold text-slate-400">Sales Volume Lift</span>
      ...
      <div className="saas-card p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-medium text-slate-400">Incremental Profit Learning</span>
        </div>
      ...
      <div className="saas-card p-5 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
            ...
            Historical Cross-Product Bundle Uplift Learning Table ({filteredBundles.length} records)
          </h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-200 text-slate-500 font-semibold uppercase tracking-wider bg-slate-50">
                ...
                <th className="py-3 px-3">Incremental Profit</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredBundles.map((b) => (
                <tr key={b.bundleId} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-3.5 px-3 font-black text-emerald-600">+₹{b.incrementalProfit.toLocaleString('en-IN')}</td>
                  <td className="py-3.5 px-3">
                    <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                      b.meaningfulUpliftFlag ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500'
                    }`}>
                      {b.meaningfulUpliftFlag ? '✅ Meaningful' : '❌ Moderate'}
                    </span>
                  </td>
